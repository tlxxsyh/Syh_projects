...
int main()
{
	...
	//创建图形窗口
	initgraph(4 * GRID_WIDTH + 5 * EDGE, 675+100);
	
	//设置背景颜色
	setbkcolor(RGB(243, 235, 212));
	cleardevice();//清屏（同步）
	//设置背景模式为透明
	...
	_tcscpy(f.lfFaceName, _T("微软雅黑"));   // 设置字体为微软雅黑
	f.lfQuality = ANTIALIASED_QUALITY;		// 设置输出效果为抗锯齿
	settextstyle(&f);						// 设置字体样式
	settextcolor(BLACK);
	outtextxy(15, 680, "汉字演变：图画文字->甲骨文->金文->大篆->");
	outtextxy(10, 710, "小篆->六国文字->隶书->楷书->行书->草书->");
	outtextxy(10, 740, "简体字(您的目标即为从图画文字演化到简体字）");
	...
	while (1)
	{
		...
		if (enddd == 1) {
			gettextstyle(&f);						// 获取当前字体设置
			f.lfHeight = 200;						// 设置字体高度为 125
			_tcscpy(f.lfFaceName, _T("宋体"));// 设置字体为“汉仪昌黎宋刻本(精修版)W”(高版本 VC 推荐使用 _tcscpy_s 函数)
			f.lfQuality = ANTIALIASED_QUALITY;		// 设置输出效果为抗锯齿
			settextstyle(&f);						// 设置字体样式
			settextcolor(RED);
			outtextxy(150, 300, "菜");
...
void showScore(Game* game2048)
{
	...
	LOGFONT f;
	gettextstyle(&f);						// 获取当前字体设置
	f.lfHeight = 90;						// 设置字体高度为 125
	_tcscpy(f.lfFaceName, _T("宋体"));// 设置字体为“汉仪昌黎宋刻本(精修版)W”(高版本 VC 推荐使用 _tcscpy_s 函数)
	f.lfQuality = ANTIALIASED_QUALITY;		// 设置输出效果为抗锯齿
	settextstyle(&f);						// 设置字体样式
	settextcolor(BLACK);
	outtextxy(25, 25, "汉字2048");
	//游戏区块
	{
	...
	for (int i = 1; i <= 4; i++)
	{
		for (int j = 1; j <= 4; j++)
		{
			...
			if (game2048->map[i][j] != 0)
			{
				...
				//设置内容
				if (str[0] == '2'&&str[1]!='5'&&(str[1] != '0' && str[2] != '4')) {
					settextstyle(30, 0, "微软雅黑");
					outtextxy(x+5 , y + higt+8, "图画文字");
				}
				else if (str[0] == '4' && str[1] != '0') {
					settextstyle(30, 0, "微软雅黑");
					outtextxy(x + 18, y + higt + 8, "甲骨文");
				}
				else if (str[0] == '8') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "金文");
				}
				else if (str[0] == '1' && str[1] == '6') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "大篆");
				}
				else if (str[0] == '3' && str[1] == '2') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "小篆");
				}
				else if (str[0] == '6' && str[1] == '4') {
					settextstyle(30, 0, "微软雅黑");
					outtextxy(x + 5, y + higt + 8, "六国文字");
				}
				else if (str[0] == '1' && str[1] == '2' && str[2] == '8') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "隶书");
				}
				else if (str[0] == '2' && str[1] == '5' && str[2] == '6') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "楷书");
				}
				else if (str[0] == '5' && str[1] == '1' && str[2] == '2') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "行书");
				}
				else if (str[0] == '1' && str[1] == '0' && str[2] == '2' && str[3] == '4') {
					settextstyle(40, 0, "微软雅黑");
					outtextxy(x + 20, y + higt + 8, "草书");
				}
				else if (str[0] == '2' && str[1] == '0' && str[4] == '2' && str[3] == '8') {
					settextstyle(30, 0, "微软雅黑");
					outtextxy(x + 18, y + higt + 8, "简体字");
				}
				else {
